...
import { form, FormField, max, min, minLength, required, requiredError, schema, submit, validate } from '@angular/forms/signals';
...
export class UserForm {
  ...
  private domains = ["example.com", "yahoo.com", "outlook.com"];
  ...
  public userForm = form(this.userModel, schema => { 
    ...
    validate(schema, ctx => {
      ...
      if(gender === "female" && age < 25) return requiredError({message: "Las mujeres no pueden tener menos de 25 años"});
      if(gender === "male" && age < 40) return requiredError({message: "Los hombres no pueden tener menos de 40 años"});
      return null;
    });
