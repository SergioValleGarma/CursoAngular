...
export class ErrorValidations {
    ...
    public getMessageErrors(): IMessageError[] {
        ...
        if (this.controlName && this.title) {
            ...
            const errorObj = Object.keys(control.errors!);
            ...
            errorObj.forEach(error => {
                switch (error) {
                    ...
                    case 'minlength':
                        messages.push({
                            type: error, message: `El campo '${this.title}' debe tener al menos 
                    ${control.errors!['minlength'].requiredLength} caracteres.`
                        });
                        break;
                    case 'pattern':
                        messages.push({
                            type: error, message: `El campo '${this.title}' no tiene el formato correcto.
                    ${control.errors!['pattern']?.requiredPattern}`
                        });
                        break;
                    case 'domainInvalid':
                        messages.push({ type: error, message: `${control.errors!['domainInvalid']}` });
                        break;
                    case 'min':
                        messages.push({
                            type: error, message: `El campo '${this.title}' debe ser mayor o igual a 
                    '${control.errors!['min']?.min}' .`
                        });
                        break;
                    case 'max':
                        messages.push({
                            type: error, message: `El campo '${this.title}' debe ser menor o igual a
